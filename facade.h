...
public:
    int file_num=1;
    int file_index=0;   //index for empty points in graphiz
    std::fstream file;      //file for graphviz
    ...
    Facade(std::string message);    //constructor
    ~Facade();     //destructor
    QString BuildFrequencyStr();    //method making string for decoding exersize
    void SetAnwserInt(int a);   //method set right answer for "write lenght of code" exersizes
    ...
    void CodingByHaffman(BinTree* head);    //method coding by Haffman
    static bool cmp(BinTree* t1, BinTree* t2);     //comparator for vector sorting
    void BuildTreeFile();       //method builds file for Graphviz
    void BuildNode(BinTree* head);  //recursive method for making file for graphviz
};
